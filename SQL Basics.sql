...
#(I had to add bank.order instead of just order because I kept getting a Syntax order https://www.w3schools.com/sql/sql_orderby.asp)
...
SELECT DISTINCT(district_id), COUNT(district_id) AS number_of_clients FROM client
WHERE district_id<10
GROUP BY district_id
ORDER BY district_id ASC;

#In the card table, how many cards exist for each type? Rank the result starting with the most frequent type.
SELECT DISTINCT(type), COUNT(type) AS number_of_cards FROM card
GROUP BY type
ORDER BY COUNT(type) DESC;

#15Using the loan table, print the top 10 account_ids based on the sum of all of their loan amounts.
SELECT account_id, SUM(amount) FROM loan
GROUP BY account_id #I wonder why i couldn't use group by amount
ORDER BY SUM(amount) DESC
LIMIT 10;

#In the loan table, retrieve the number of loans issued for each day, before (excl) 930907, ordered by date in descending order.
SELECT DISTINCT(date), COUNT(*) AS number_of_loans_issued FROM loan
WHERE date < 930907
GROUP BY date
ORDER BY date DESC;

#In the loan table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your output.
SELECT date, duration, COUNT(amount) AS number_of_loans_issued FROM loan
WHERE date BETWEEN 971201 AND 971231
GROUP BY date, duration
ORDER BY date, duration; #explained in class 25/02
...
#Rachelled helped us for the last two
